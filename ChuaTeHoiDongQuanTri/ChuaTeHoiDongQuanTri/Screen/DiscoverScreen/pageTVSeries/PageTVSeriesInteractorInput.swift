...
class PageTVSeriesInteractorInput {
    ...
    private func getSearchResult(area : String,
                                 ...
                                 subtitles : String,
                                 order : String) {
        APIService.shared.getAdvancedSearching(params: DiscoveryParams.TVSeries.params, area: area, category: category, year: year, subtitles: subtitles, order: order) { [weak self] response, error in
            guard let `self` = self, let unwrappedData = response else { return }
